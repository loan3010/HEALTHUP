...
router.put('/:id/admin-reply', async (req, res) => {
  try {
    ...
    // ✅ FIX: Gửi thông báo cho user nếu review có userId
    if (review.userId) {
      try {
        ...
        await Notification.create({
          ...
          message:   `HealthUp đã phản hồi đánh giá của bạn về "${productName}"`,
          icon:      '💬',
          productId: review.productId,
          isRead:    false,
        });
      } catch (e) {
        console.error('Notify user review reply error:', e);
      }
    }
